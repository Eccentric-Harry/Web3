        ...
        <div className="mt-6 space-y-6">
          {wallets.map(({ address, privateKey, recipient, amount, showInputs }, index) => (
            <div key={index} className={`${addressBoxClass} p-4 sm:p-6`}>
              <span className="block truncate text-center text-sm sm:text-base">{`Ethereum - ${address}`}</span>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:space-x-4 mt-4">
                <button onClick={() => fetchBalance(address)} className="btn btn-primary w-full sm:w-auto mb-4 sm:mb-0">
                  Get Balance
                </button>
